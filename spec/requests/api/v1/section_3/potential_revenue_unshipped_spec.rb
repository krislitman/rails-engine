...
RSpec.describe 'Report of orders which have not shipped' do
  before :each do 
    ...
    @item1 = create(:item)
    @invoice1 = create(:invoice, merchant_id: @merchant.id, customer_id: @customer.id, status: 'shipped')
    ...
    @item2 = create(:item)
    @invoice2 = create(:invoice, merchant_id: @merchant2.id, customer_id: @customer2.id, status: 'failed')
  ...
  describe 'Ranked by total revenue DESC' do
    ...
    it 'Should default to 10 orders returned if no param given' do
      # order need revenue, so invoice items, items, transactions, invoices
      # unshipped orders will be invoices with a status that doesnt equal shipped
      ...
      expect(expected[:data].last[:id].to_i).to eq(@invoice1.id)
    end
    it 'Can return any quantity of orders'
  end
end