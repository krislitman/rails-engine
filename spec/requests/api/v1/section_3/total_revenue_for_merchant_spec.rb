...
RSpec.describe 'Total revenue for a merchant' do
  it 'Can return a merchants total revenue based on merchant id' do
    ...
    invoice1 = create(:invoice, customer_id: customer.id, merchant_id: merchant.id, status: 0)
    invoice2 = create(:invoice, customer_id: customer.id, merchant_id: merchant.id, status: 0)
    invoice3 = create(:invoice, customer_id: customer.id, merchant_id: merchant.id, status: 0)
    invoice4 = create(:invoice, customer_id: customer.id, merchant_id: merchant.id, status: 0)
